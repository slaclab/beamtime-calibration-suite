    ...
    for nstep, step in enumerate(stepGen):
        ...
        for nevt, evt in enumerate(step.events()):
            ...
            if beamEvent or skip283:
                if cn.special is not None and "CommonMode" in cn.special:
                    ...
                    if "noCommonMode" in cn.special:
                        ...
                        frames = cn.regionCommonModeCorrection(
                            frames[0], cn.regionSlice, commonModeCut
                        )
                else:
                    frames = cn.getRawData(evt, gainBitsMasked=True)
                    if cn.special is not None and "parity" in cn.special:
                        if cn.getPingPongParity(frames[0][144:224, 0:80]) == (
                            "negative" in cn.special
                        ):
                            continue
                    try:
                        frames = frames[0]
            ...
            for i, p in enumerate(cn.singlePixels):
                if i >= 0 and i < len(statsArray) and len(p) >= 2:
                    print("a")
        ...
        if cn.special is not None and "slice" in cn.special:
            ...
            logger.info(
                "mean, median noise:" + str(noise.mean()) + str(np.median(noise))
            )
            means = means[cn.regionSlice]
        else:
            pass

        meanRmsFileName = "%s/CalcNoiseAndMean_%s_rms_r%d_step%s.npy" % (
            cn.outputDir,
            cn.label,
            cn.run,
            nstep,
        ...
        meanFileName = "%s/CalcNoiseAndMean_mean_%s_r%d_step%s.npy" % (
            cn.outputDir,
            cn.label,
            cn.run,
            nstep,
        ...
        for i, p in enumerate(cn.singlePixels):
            try:
                ...
                np.save(
                    meanCorrelationFileName,
                    statsArray[i].corr(
                        statsArray[i].mean()[p[1], p[2]],
                        statsArray[i].rms()[p[1], p[2]],
                    )[cn.regionSlice],
                )
