...
##    for nstep, step in enumerate (cn.ds.steps()):
    for nstep, step in enumerate (stepGen):
            ...
            if beamEvent or skip281:
                if cn.special is not None and 'CommonMode' in cn.special:
                    ...
                    if 'noCommonMode' in cn.special:
                        ...
                        frames = cn.regionCommonModeCorrection(frames[0], cn.regionSlice, 2.)
                else:
        ...
        for i, p in enumerate(cn.singlePixels):
            try:
                np.save("%s/CalcNoiseAndMean_correlation_pixel_%d_%d_%s_r%d_step%s.npy" %(cn.outputDir, p[1], p[2], cn.label, cn.run, nstep), statsArray[i].corr(statsArray[i].mean()[p[1],p[2]], statsArray[i].rms()[p[1],p[2]])[cn.regionSlice])
            except:
                ## probably rms = 0.
                continue
