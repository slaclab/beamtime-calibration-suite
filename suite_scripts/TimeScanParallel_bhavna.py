...
class TimeScanParallel(BasicSuiteScript):
    ...
    def plotData(self, rois, pixels, delays, label):
       ...
       # for i, roi in enumerate(self.ROIs):
        ...
        plt.savefig("%s/%s_r%d_c%d_%s_pixel%d_all.png" %(self.outputDir,self.__class__.__name__, self.run, self.camera, label, i))
           # plt.clf()
    ...
    def analyze_h5(self, dataFile, norm, label):
        ...
        delays = (np.array([k for k in a.keys()])).astype(int)
        delays.sort()
        delays = list(map(str, delays))
        d = np.array([a[k] for k in delays])
        delays = np.array([eval(d) for d in delays])
...
##    for nstep, step in enumerate (tsp.ds.steps()):
    for nstep, step in enumerate (stepGen):
        ...
        ratioSums = np.zeros(len(tsp.ROIs) + len(tsp.singlePixels)).astype(np.uint32)
