    ...
    for run in runs:
        for rb in range(nRowBlocks):
            ...
            for cb in range(nColBlocks):
                ...
                wrapString = (
                    "python -u -m mpi4py.run SimpleClustersParallelSlice.py -r %d --fakePedestal $OUTPUT_ROOT/dark/CalcNoiseAndMean_mean__r%d_step0.npy --seedCut 4 --label m%d_r%d-r%d-c%d-c%d --special slice --analyzedModules '[%d]' --regionSlice '[%d,%d,%d,%d,%d,%d]'"
                    % (run, darkRun, m, rStart, rStop, cStart, cStop, m, 0, lastModule, rStart, rStop, cStart, cStop)
                )
                wrapString += extraWrapString
                command = [
                    "sbatch",
                    "-o",
                    "r%d_m%d_r%d-r%d-c%d-c%d.log" % (run, m, rStart, rStop, cStart, cStop),
                    "-p",
                    "%s" % (queue),
                    "--account",
                    "lcls:%s" % (account),
