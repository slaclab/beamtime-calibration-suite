...
if (combined_data.ndim==4):
    ...
    def get_entry():
        while True:
            try:
                entry = int(input("Which ASIC do you want to study? 0, 1, 2, or 3: "))
                if entry not in VecASIC:
                    raise ValueError("Invalid ASIC number. Please enter 0, 1, 2, or 3.")
                return entry
            except ValueError as e:
                print(e)


    def concatenate_matrices(*args):
        return np.hstack(args)


    #entry = get_entry()
    print("Looking at all ASICs")
   
    axis_to_concatenate = 1  # Assuming you want to concatenate along the second axis

    combined_data = np.concatenate([combined_data[:, i, :, :] for i in range(combined_data.shape[axis_to_concatenate])], axis=axis_to_concatenate)
...
#    for ic in range(30,31):
        ...
        if False:
            ...
            plt.plot(min(X[Y==min(Y[len(Y)//2:-1])]),y_new_2ndpart[np.argmin(smoothed_data[(len(smoothed_data)//2):-1])],'y*')
