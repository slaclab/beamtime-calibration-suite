import numpy as np
import pytest
from calibrationSuite.ancillaryMethods import makeProfile, plotProfile, selectedClusters, getEnergeticClusters, getSmallSquareClusters, getSmallClusters, getSquareClusters, getMatchedClusters, goodClusters, getClusterEnergies
...
def test_getSmallSquareClusters():
    clusters = np.array([[1, 1, 1, 1, 4, 1], [2, 1, 2, 2, 3, 1],
...
def test_getMatchedClusters():
    clusters = np.array([[1, 1, 1, 6], [2, 1, 2, 6],
                         [2, 2, 2, 6], [3, 4, 3, 6]])
    dimension = 'module'
    n = 1
