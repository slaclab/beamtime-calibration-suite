...
class AnalyzeH5(object):
    ...
    def analyzeSimpleClusters(self, clusters):
        ...
        for i in range(rows):
            for j in range(cols):
                pixel = np.bitwise_and((clusters[:,:,1] == i), (clusters[:,:,2] 
== j))
                small = np.bitwise_and((clusters[:,:,3]<4), (clusters[:,:,4]==1))
                smallPixel = np.bitwise_and(small, pixel)
                pixelEcut0 = np.bitwise_and(smallPixel, energy>self.lowEnergyCut) ## adjusted due to gains not making sense
                pixelEcut = np.bitwise_and(pixelEcut0, energy<self.highEnergyCut) ## would be good to get rid of these entirely when things make sense
                ...
                if nPixelClusters > 5:
                    ...
                    plt.savefig("%s/%s_r%d_c%d_r%d_c%d_%s_E.png" %(self.outputDir,self.__class__.__name__, self.run, self.camera, i, j, self.label))
                    plt.close()
                
        np.save("%s/%s_r%d_c%d_r%d_c%d_%s_fitInfo.npy" %(self.outputDir,self.__class__.__name__, self.run, self.camera, i, j, self.label), fitInfo)
        gains = fitInfo[:,:,2]
