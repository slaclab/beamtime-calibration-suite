    ...
    def plotData(self, clusters, label):
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                pixelEcut0 = np.bitwise_and(smallPixel, energy>1) ## adjusted due to gains not making sense
                pixelEcut = np.bitwise_and(pixelEcut0, energy<12) ## would be good to get rid of these entirely when things make sense
                ...
                if nPixelClusters > 5:
                    ...
                    plt.savefig("%s/%s_r%d_c%d_r%d_c%d_%s_E.png" %(self.outputDir,self.__class__.__name__, self.run, self.camera, i, j, label))
                    plt.close()
                
        np.save("%s/%s_r%d_c%d_r%d_c%d_%s_fitInfo.npy" %(self.outputDir,self.__class__.__name__, self.run, self.camera, i, j, label), fitInfo)
        gains = fitInfo[:,:,2]
        ...
        plt.savefig("%s/%s_r%d_c%d_%s_gainDistribution.png" %(self.outputDir,self.__class__.__name__, self.run, self.camera, label))
