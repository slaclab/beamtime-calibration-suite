...
class PsanaBase(object):
    def __init__(self, analysisType='Scan'):
        ...
        ## temp ##
        self.h5File = h5py.File(self.args.files)
        if 'run' not in dir(self):
            self.run = 666666
    ...
    def setupPsana(self): ## should rename to setupBase or whatever
        print("in fake setupPsana; get info from h5 and fake stuff")
        print("h5:", self.args.files)
        if False:
            self.ds = DataSource(files=[self.args.files]) ## to make smalldata happy
            print("data source is", self.ds)
        else:
            self.ds = DataSource(exp='rixx1005922', run=6)##66)
